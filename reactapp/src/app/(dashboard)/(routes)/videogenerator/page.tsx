...
import FetchVideos from "@/components/video/fetchvideo";
...
const VideoGeneratorPage = () => {
    ...
    const generateVideo = async (prompt: string) => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/video/generate-video/`, {
                method: "POST",
                headers: {
    ...
    const handleSubmit = async () => {
        ...
        const generatedVideoUrl = await generateVideo(userInput);
    ...
    return (
        <div className="flex flex-col h-screen">
            ...
            <Heading
                ...
                description={
                <>
                    Générer des videos à partir de texte (lien disponible 1h) ! video produite par le model{" "}
                    <a href="https://replicate.com/lucataco/animate-diff\" target="_blank\" rel="noopener noreferrer\" className="text-blue-500 underline">
                        Animate-diff
                    </a>
                </>
                }
            ...
            <div className="flex flex-col flex-grow p-4 border-0 lg:p-8">
                <Card className="flex-grow overflow-y-auto dark:border-0 border-white p-2 mb-4">
                    <FetchVideos refreshKey={refreshKey} />
                </Card>
                                    ...
                                    <FormItem className={"col-span-12 lg:col-span-10"}>
                                        <FormControl className={"m-0 p-0"}>
                                            <Input
                                                ...
                                                disabled={isLoading}
                                                placeholder={"insérer ici le prompt de la video que vous souhaitez générer ?"}
                                                {...field}
...
export default withAuth(VideoGeneratorPage);
